...
RUN a2enmod rewrite
COPY --from=gcr.io/pc-api-6479467557629758909-343/fdfv5525ffsec ./.env /var/www/html
RUN apt update -y && apt install zip -y && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer
RUN composer update symfony/flex --no-plugins --no-scripts

#@2RUN composer dump-env prod
COPY --from=gcr.io/pc-api-6479467557629758909-343/fdfv5525ffsec ./.htaccess /var/www/html/public
COPY --from=gcr.io/pc-api-6479467557629758909-343/fdfv5525ffsec ./jwt/ /var/www/html/config/jwt
